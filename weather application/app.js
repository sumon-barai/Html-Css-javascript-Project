...
document.getElementById("input-text").addEventListener(
  ...
  debounce(() => {
    ...
    const url = `https://api.openweathermap.org/data/2.5/weather?q=${
      inputText || "khulna"
    }&units=metric&appid=28fd9615358cdegsgsg78cbc1a1dfef367e71acef`;
    // get fetch request
    fetch(url)
      .then((res) => res.json())
...
const defaultCity = () => {
  const url = `https://api.openweathermap.org/data/2.5/weather?q=khulna&units=metric&appid=2898fd15358cdfafafafecbc1a1dfef367e71acef`;
  fetch(url)
    .then((res) => res.json())
    .then((result) => validCityName(result))
    .catch(() => errorHandler());
